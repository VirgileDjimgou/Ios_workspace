...
class Country : NSObject {
  ...
  static func readFromBundle() -> [Country] {
    ...
    if let path =  Bundle.main.path(forResource: "bundesländer",
                                    ofType: "txt")
    {
      do {
        ...
        for line in lines {
          ...
          if columns.count == 4 {
            let name = columns[0]
            let area =
            Double(fmt.number(from: columns[1]) ?? 0.0)
            let pop = Int(columns[2]) ?? 0
            let cap = columns[3]
            data.append(Country(name: name, population: pop,
                                area: area, capital: cap))
          }
        }  // Ende for
      } catch _ {}
    }      // Ende if let
