...
class SettingsVC: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    // Initialisierung von calcVC
    calcVC = tabBarController!.viewControllers?.first as? CalcVC
    let cc = calcVC.cc
    picker1.dataSource = self
    picker1.delegate = self
    picker1.selectRow(
      cc?.currencies.index(of: calcVC.cur1) ?? 0,
      inComponent: 0,
      animated: true)
    picker2.dataSource = self
    picker2.delegate = self
    picker2.selectRow(
      cc?.currencies.index(of: calcVC.cur2) ?? 0,
      inComponent: 0,
      animated: true)
  }
  
  // Picker-View-Daten neu laden
...
//  }
    ...
    reusing view: UIView?) -> UIView
  {
    if view != nil {
      ...
      let lbl = UILabel(
        frame: CGRect(x: 90, y: 0, width: 60, height: 50))
      lbl.text = txt
