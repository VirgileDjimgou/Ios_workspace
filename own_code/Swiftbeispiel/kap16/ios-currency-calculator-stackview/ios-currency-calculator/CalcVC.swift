...
class CalcVC: UIViewController {
  ...
  var cur1 = "EUR" {
    ...
    didSet {
      lbl1?.text = cur1
      img1?.image = UIImage(named: CurCalc.getCountryFromCurrency(cur1))
      txt1?.text = ""
      txt2?.text = ""
      userdef.setObject(cur1, forKey: "currency1")
    }
  }
  var cur2 = "USD" {
    didSet {
      lbl2?.text = cur2
      img2?.image = UIImage(named: CurCalc.getCountryFromCurrency(cur2))
      txt1?.text = ""
      txt2?.text = ""
      userdef.setObject(cur2, forKey: "currency2")
    }
  }
  ...
  override func viewDidLoad() {
    ...
    cur1 = userdef.stringForKey("currency1") ?? "EUR"
    cur2 = userdef.stringForKey("currency2") ?? "USD"
  ...
  func refreshData() {
    ...
    let formattedDate = dateformatter.stringFromDate(cc.lastupdate)
  ...
  @IBAction func txtEditingChanged(sender: UITextField) {
    var value:Double
    if let decimal = formatter.numberFromString(sender.text!) {
    ...
    if sender === txt1 {
      ...
      txt2.text = formatter.stringFromNumber(
        cc.convert(value, from:cur1, to: cur2))
    } else if sender === txt2 {
      // Umrechnung in Währung eins
      txt1.text = formatter.stringFromNumber(
        cc.convert(value, from:cur2, to: cur1))
    }
  }
